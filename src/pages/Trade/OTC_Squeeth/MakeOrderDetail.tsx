...
export default function MakeOrderDetailSimple({ paymentTokenBalance, squeethBalance }: TradeDetailProps) {
  ...
  const inputToken = useMemo(() => (action === TradeAction.Buy ? paymentToken : squeeth), [paymentToken, action])

  const outputToken = useMemo(() => (action === TradeAction.Buy ? squeeth : paymentToken), [paymentToken, action])
  ...
  const { allowance: oTokenAllowance, approve: approveOTokenZX } = useUserAllowance(squeeth.id, Spenders.ZeroXExchange)
  ...
  return (
    <>
      ...
      <TokenBalanceEntry
        ...
        symbol={`${paymentToken.symbol} / ${squeeth.symbol}`}
      />
