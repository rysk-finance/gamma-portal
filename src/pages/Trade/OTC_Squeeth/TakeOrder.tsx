...
export default function TakerOrder({ squeethBalance, paymentTokenBalance }: TakerOrderProps) {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const makerAsset =
        order.makerToken.toLowerCase() === squeeth.id
          ? squeeth
          : order.makerToken.toLowerCase() === weth.id
          ? weth
          : undefined

      const takerAsset =
        order.takerToken.toLowerCase() === squeeth.id
          ? squeeth
          : order.takerToken.toLowerCase() === weth.id
          ? weth
          : undefined
